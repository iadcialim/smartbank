...
export class PaymentMethodsService {
   ...
   */
  static async getUserPaymentMethodsByType(type: 'card' | 'bank'): Promise<ApiResponse<PaymentMethod[]>> {
    try {
      const { data: { user }, error: authError } = await supabase.auth.getUser()
   ...
   */
  static async getDefaultPaymentMethod(): Promise<ApiResponse<PaymentMethod | null>> {
    try {
      const { data: { user }, error: authError } = await supabase.auth.getUser()
   ...
   */
  static async getPaymentMethodById(paymentMethodId: string): Promise<ApiResponse<PaymentMethod>> {
    try {
      const { data: { user }, error: authError } = await supabase.auth.getUser()
   ...
   */
  static async createPaymentMethod(paymentMethodData: Omit<PaymentMethodInsert, 'user_id'>): Promise<ApiResponse<PaymentMethod>> {
    try {
      const { data: { user }, error: authError } = await supabase.auth.getUser()
   ...
   */
  static async updatePaymentMethod(paymentMethodId: string, updates: PaymentMethodUpdate): Promise<ApiResponse<PaymentMethod>> {
    try {
      const { data: { user }, error: authError } = await supabase.auth.getUser()
   ...
   */
  static async setDefaultPaymentMethod(paymentMethodId: string): Promise<ApiResponse<PaymentMethod>> {
   ...
   */
  static async deactivatePaymentMethod(paymentMethodId: string): Promise<ApiResponse<PaymentMethod>> {
   ...
   */
  static async getPaymentMethodStats(): Promise<ApiResponse<{
    ...
    try {
      ...
      const totalCards = paymentMethods.filter(pm => pm.type === 'card').length
      const totalBankAccounts = paymentMethods.filter(pm => pm.type === 'bank').length
